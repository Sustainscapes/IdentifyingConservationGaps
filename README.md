...
This README documents the spatial data processing workflow used to
derive the terrestrial map and summary statistics presented in Fig. 1
and Table S2 of the manuscript. in the supplementary material.
...
This yields a total terrestrial area of 43,144.85 km².
...
shown in turquoise (protected areas). These areas encompass areas under
...
In Fig. 1, areas under protection schemes where biodiversity is directly
compromised by human land uses (e.g. intensive agriculture, production
forestry, infrastructure) are shown in **orange** (“Areas compromised by
...
- agricultural land (fields in rotation and permanent grasslands not
  covered by section 3 protection CF. Nature protection act),
- production forest (tree-covered areas not designated as unmanaged
  forest or section 3 protection CF. Nature protection act), and
- infrastructure (roads, buildings and other built-up areas).

These biodiversity-compromising areas are identified for each protection
scheme and combined into a single raster used to derive the production
category in Fig. 1.
...
rotation (`INT_AGG`), grasslands not covered by Section 3 protection
...
are classified as **insufficient legal protection** (yellow in Fig. 1).

On land, this primarily concerns **private areas protected under Section
3 of the Danish Nature Protection Act**. This protection scheme mainly
covers open habitats (e.g. meadows, heaths, and wetlands) and does not
include forest habitats. On privately owned land, there are no mandatory
management requirements to maintain these open habitats. As a result,
many areas may gradually transition to forest through natural succession
and effectively “grow out” of protection, undermining long-term
biodiversity conservation.

To operationalise this, we identify areas that:

1.  are covered by **exactly one** protection scheme,
2.  fall under **Section 3 protection** (Nature Protection Act), and  
...
### Identifying private Section 3 areas with a single protection scheme
...
Next, we read the Section 3 protection raster. We convert it to a binary
layer where 1 indicates Section 3 protection and 0 indicates “unknown”
or lakes.
...
# 0–3, 5  = Section 3 categories kept as 1
...
legal guarantees are not consistently documented). In these cases,
criteria C1–C5 cannot be assessed conclusively using spatial data alone.

In the manuscript, areas that **require individual assessment** are
those where the available spatial data do not allow us to evaluate
criteria C1–C5 conclusively.

Operationally, we classify pixels as “requires individual assessment” if
they fall within a mapped protection scheme and are not already
classified as fully contributing protected areas, production/compromised
areas, or insufficient legal protection.

### Identifying areas that require individual assessment

We use the `Subclasses` layer to identify pixels that fall within at
least one mapped protection scheme and transform it into a binary layer
that we will later use to build the harmonised classification.

Any pixel within the protection schemes will be considered a candidate
for being in the **requires individual assessment (RIA)** category.
...
2000 site that is also a Section 3 area). To produce the final map in
...
# 3: Insufficient legal protection (private Section 3, single-scheme protection)
...
This section computes the total area (km²) and share (percentage) of
Denmark’s terrestrial area in each category defined in `FinalLayer`.
Proportions are calculated relative to Denmark’s total terrestrial area
(`Area_DK`).
...
This table reproduces the proportions shown in the doughnut chart in
Fig. 1, with small differences due to rounding.
...
Table S2 in the supplementary material summarises, for each protection
scheme:
...
- the area and percentage of that scheme that overlaps with  
  **(i)** infrastructure,  
  **(ii)** agriculture, and  
  **(iii)** managed forest (i.e. production forest).
...
- one column per protection scheme or pressure, coded as `"Yes"` or
  `NA`, and  
...
protection scheme (i.e. `Subclasses` is not `NA`), and then create
`"Yes"/NA` indicators for each variable used in Table S2:
...
- total area (km² and percentage of Denmark), and  
