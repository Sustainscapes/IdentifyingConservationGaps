...
This README focuses on the spatial data processing steps used to derive the terrestrial map and summary statistics for Fig. 1 and table S2 in the supplementary material.
...
This yields a total terrestrial area of `r prettyNum((round(Area_DK, 2)), big.mark = ",")` square kilometers.
...
In fig 1, areas that **fully contribute** to the 30% land target are shown in turqouise (protected areas). These areas encompass areas under protection schemes with no landuses compromising biodiversity and that meet all criteria C1–C5 for protected areas (Table 1 in the manuscript). 
...
These areas compromising biodiversity are identified for each protection scheme. These areas are combined into a resulting raster that is then used to derive the orange category in fig. 1.
...
We restrict the analysis to pixels that fall inside at least one protection subclass (i.e. `Subclasses` is not `NA`), and then create `"Yes"/NA` dummies for each variable we want to use in S2:
