...
This README documents the spatial data processing workflow used to derive the terrestrial map and summary statistics presented in Fig. 1 and Table S2 of the manuscript. in the supplementary material.
...
We also use [`dplyr`](https://cran.r-project.org/package=dplyr) and [`purrr`](https://cran.r-project.org/package=purrr) for data manipulation and iteration,  
[`stringr`](https://cran.r-project.org/package=stringr) for text wrapping in legend labels, and the GitHub package  
[`BDRUtils`](https://github.com/derek-corcoran-barrios/BDRUtils) (BiodiversitetRåders utilities) for helper functions used in Table S2 (notably `exclusivity_table()`).
...
This yields a total terrestrial area of `r prettyNum((round(Area_DK, 2)), big.mark = ",")` km².

## Built-up areas

To identify areas that genuinely contribute to the 30% target, we exclude built-up areas (infrastructure and urban land), which cannot contribute to biodiversity protection. Built-up areas were extracted from the land-cover layer in [basemap](https://envs.au.dk/en/research-areas/society-environment-and-resources/land-use-and-gis/basemap).

Here, `BuildUp` is a raster where  
0 = built-up (infrastructure, urban areas),  
1 = other land.  

Pixels classified as built-up (`BuildUp == 0`) are removed from layers intended to represent contributing protected areas, and are flagged as compromised where relevant (e.g. in the production/pressure layers).
...
In Fig. 1, areas that **fully contribute** to the 30% land target are shown in turquoise (protected areas). These areas encompass areas under protection schemes with no landuses compromising biodiversity and that meet all criteria C1–C5 for protected areas (Table 1 in the manuscript). 
...
Below we show how these layers are read, harmonised and combined into a single raster representing all terrestrial areas that contribute to the 30% target.

To track overlaps between protection schemes, each scheme is encoded as a binary layer with a **distinct base value** (Nature National Park = 1, Private Nature Foundations = 10, State-owned Unmanaged Forests = 100). When these layers are summed, each combination of schemes yields a unique value (e.g. 1 = Nature National Park only; 11 = Nature National Park + private nature foundation; 111 = all three schemes).
...
In the original raster, Nature National Parks are coded as 0 and all other pixels are `NA`; these are recoded to a simple presence/absence layer.
...
In Fig. 1, areas under protection schemes where biodiversity is directly compromised by human land uses (e.g. intensive agriculture, production forestry, infrastructure) are shown in **orange** (“Areas compromised by infrastructure, forestry, or agriculture”).

These areas are identified by combining:

* agricultural land (fields in rotation and permanent grasslands not covered by section 3 protection CF. Nature protection act),
* production forest (tree-covered areas not designated as unmanaged forest or section 3 protection CF. Nature protection act), and
* infrastructure (roads, buildings and other built-up areas).

These biodiversity-compromising areas are identified for each protection scheme and combined into a single raster used to derive the production category in Fig. 1.

### Reading and combining the layers

We use a raster (`Subclasses`) that identifies areas falling within at least one mapped protection scheme. Some of these schemes allow continued agricultural or forestry production, such as fields in rotation (`INT_AGG`), grasslands not covered by Section 3 protection (`PGR_out_of_P3`), and production forest (`Drevet Skov`). Although formally included within protection schemes, these areas still function as production landscapes and are therefore merged into a single layer of production/compromised areas.
...
We then add built-up areas (from `BuildUp`, read above). Pixels that are both within the `Subclasses` raster (i.e., part of at least one mapped protection scheme) and classified as built-up (`BuildUp == 0`) are also flagged as production/compromised areas. In other words, built-up areas override any protection designation and are always classified as production.
...
This layer corresponds to the production/compromised category used in the main classification and visualised in Fig. 1.

## Insufficient legal protection

Areas under protection schemes that do not meet the criteria for long-term legal protection (Criterion C1) or other key criteria (C2–C5) are classified as **insufficient legal protection** (yellow in Fig. 1).

On land, this primarily concerns **private areas protected under Section 3 of the Danish Nature Protection Act**. This protection scheme mainly covers open habitats (e.g. meadows, heaths, and wetlands) and does not include forest habitats. On privately owned land, there are no mandatory management requirements to maintain these open habitats. As a result, many areas may gradually transition to forest through natural succession and effectively “grow out” of protection, undermining long-term biodiversity conservation.
...
Next, we read the Section 3 protection raster. We convert it to a binary layer where 1 indicates Section 3 protection and 0 indicates “unknown” or lakes.
...
We then read in the ownership data, convert it to a binary “private vs. other” layer, and resample it to match the resolution and extent of the Section 3 raster:
...
Some protection schemes cannot be fully evaluated with the available information (for example, because management prescriptions or long-term legal guarantees are not consistently documented). In these cases, criteria C1–C5 cannot be assessed conclusively using spatial data alone.

In the manuscript, areas that **require individual assessment** are those where the available spatial data do not allow us to evaluate criteria C1–C5 conclusively.

Operationally, we classify pixels as “requires individual assessment” if they fall within a mapped protection scheme and are not already classified as fully contributing protected areas, production/compromised areas, or insufficient legal protection.

### Identifying areas that require individual assessment

We use the `Subclasses` layer to identify pixels that fall within at least one mapped protection scheme and transform it into a binary layer that we will later use to build the harmonised classification.

Any pixel within the protection schemes will be considered a candidate for being in the **requires individual assessment (RIA)** category.
...
```{r final_harmonised_layer, eval = FALSE}
...
```{r final_harmonised_save, eval = FALSE}
...
This section computes the total area (km²) and share (percentage) of Denmark’s terrestrial area in each category defined in `FinalLayer`. Proportions are calculated relative to Denmark’s total terrestrial area (`Area_DK`).
...
```{r, echo=FALSE}
area_tbl <- readxl::read_xlsx("area_tbl.xlsx")
knitr::kable(
  area_tbl,
  format    = "pipe", 
  digits   = 2
...
This table reproduces the proportions shown in the doughnut chart in Fig. 1, with small differences due to rounding.
...
```{r map_final_layer, eval = TRUE, cache=TRUE}
...
Table S2 in the supplementary material summarises, for each protection scheme:
...
1. build a multi-layer raster stack with all relevant schemes and pressures,  
2. use `terra::crosstab()` to count how many pixels fall in each combination,  
3. convert pixel counts to area in km², and  
4. use the helper `exclusivity_table()` function from **BDRUtils** to summarise overlaps scheme by scheme.


```{r readDataS2, echo = TRUE, eval=FALSE}
...
We then use `terra::crosstab()` to obtain, for every unique combination of categories across these layers, the number of pixels (`n`). This is saved once and reused in the rest of the workflow:


```{r crosstabcalc, echo = T, eval=FALSE}
...
- one column per protection scheme or pressure, coded as `"Yes"` or `NA`, and  
- a column called `Area_sq_Km` giving the area of each row in km².

Because all rasters are at **10 m × 10 m** resolution, each pixel represents  
\(100 \, \text{m}^2 = 0.0001 \, \text{km}^2\). We therefore convert `n` pixels to km² as `n * 100 / 1e6`.

We restrict the analysis to pixels that fall inside at least one protection scheme (i.e. `Subclasses` is not `NA`), and then create `"Yes"/NA` indicators for each variable used in Table S2:
...
For a given protection scheme (e.g. `"Natura2000"`), we use `exclusivity_table()` three times, each time pairing the scheme with one pressure variable:
...
We then compute the share of the scheme affected by each pressure and, using `Area_DK` computed above, the share of Denmark covered by the scheme.


```{r, helperFunctionLine}
...
Finally, we apply `make_S2_line()` to each protection scheme of interest using `purrr::map2_dfr()`. The first vector (`scheme_vec`) lists the column names in `ForS2Table`, and the second vector (`nice_vec`) provides the labels that appear in the final table.


```{r tablemake, message=FALSE, warning=FALSE, cache=TRUE}
...
`S2_body` reproduces the structure of Table S2 in the manuscript, with one row per scheme and columns giving:

- total area (km² and percentage of Denmark), and  
- area and percentage of each scheme overlapped by infrastructure, agriculture and managed forest.
...
knitr::kable(
format = "pipe",
align = "lrrrrrrrrr" # adjust if you change columns
)
```
